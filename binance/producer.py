...
def get_exchange_rates():
    url = "https://api.exchangerate-api.com/v4/latest/btc"
    response = requests.get(url)
    if response.status_code == 200:
        data = response.json()
        usd_to_btc_rate = data['rates']['USD']
        eur_to_btc_rate = data['rates']['EUR']
        return usd_to_btc_rate, eur_to_btc_rate
    else:
...
def produce():
    cnt = 0
    while cnt < 30:
        usd_to_btc_rate, eur_to_btc_rate = get_exchange_rates()
        if usd_to_btc_rate is not None and eur_to_btc_rate is not None:
            usd_price_in_btc = format_price(1 / usd_to_btc_rate)
            eur_price_in_btc = format_price(1 / eur_to_btc_rate)

            current_time = datetime.now().isoformat()

            dollar_data = CurrencyCreate(
                time=current_time,
                price=usd_price_in_btc,
                coin="usd" 
            )
            euro_data = CurrencyCreate(
                time=current_time,
                price=eur_price_in_btc,
                coin="eur"
            )
