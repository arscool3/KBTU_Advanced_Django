...
def get_all_projects():
    # projects = session.query(ITProject).all()
    # return projects
    with get_db() as session:
        db_projects = session.query(db.ITProject).all()
        projects = []
...
@app.post("/itprojects")
def create_project(investor_id, initial_budget):
    with get_db() as session:
        session.add(db.ITProject(investor_id=investor_id, initial_budget=initial_budget))
    produce_log({
