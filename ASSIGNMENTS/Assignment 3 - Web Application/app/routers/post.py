...
def get_posts(db: Session = Depends(get_db), user_id: int = Depends(auth.get_current_user)):
...
def create_posts(post: schemas.PostCreate, db: Session = Depends(get_db), current_user: int = Depends(auth.get_current_user)):
