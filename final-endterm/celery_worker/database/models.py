...
class Person(Base):
    ...
    id: Mapped[_id]
    current_location_id: Mapped[int] = mapped_column(sqlalchemy.ForeignKey("locations.id"))
    current_location = relationship("Location", back_populates="people")
    current_road_id: Mapped[int] = mapped_column(sqlalchemy.ForeignKey("roads.id"))
    current_road = relationship("Road", back_populates="people")
    history = relationship("History", back_populates="person")
    current_speed: Mapped[int] = mapped_column(sqlalchemy.Integer, default=0, insert_default=True)
