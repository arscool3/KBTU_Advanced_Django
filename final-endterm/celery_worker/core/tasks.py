...
def calculate_traffic_task(traffic_schema: dict):
    ...
    try:
        with session.begin():
            ...
            previous_road_location = cache.get(f"{traffic_data.person_id}-current-road").decode("utf-8")

            logger.info("Previous road location: %s", previous_road_location)

            if previous_road_location is not None:
                if int(previous_road_location) != approximate_road_location.id:
                    prev_road = session.get(Road, previous_road_location)
                    ...
                    calculated_rate = round(
                        ((len(prev_road.people) / prev_road.max_num_of_cars)
                         * (av_speed / prev_road.max_speed))
                        * 10
                    )
            ...
            calculated_rate = round(
                ((len(approximate_road_location.people) / approximate_road_location.max_num_of_cars)
                 * (av_speed / approximate_road_location.max_speed))
                * 10
            )
